...
try {
    if (product.getImgs() != null)
        Picasso.with(context).load(Constants.PRODUCT_IMG_PATH + product.getImgs().get(0).getImg_url()).centerInside().resize(300, 300).error(R.drawable.baby_bg).into(holder.imageView);
